...
def train(df, df_balanced, plane, n_classes):
    ...
    for fold, (trn_idx, val_idx) in enumerate(skf.split(range(len(study_ids)))):
        ...
        model = RSNA24Model(model_config.MODEL_NAME, n_classes=n_classes, pretrained=PRETRAINED)
        if RETRAIN:
            fname = f'{model_config.MODEL_PATH}/{plane}-best_wll_model_fold-{fold}.pt'
        ...
        weights = torch.tensor([1.0, 2.0, 4.0] * model_config.N_LABELS)
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            with tqdm(train_dl, leave=True, desc=f"Train {plane.upper()} Fold {fold}/{N_FOLDS}, Epoch {epoch}/{EPOCHS}") as pbar:
                ...
                for idx, (x, t) in enumerate(pbar):
                    ...
                    with autocast:
                        loss = 0
                        y = model(x)
                        loss = loss + criterion(y, t) / model_config.N_LABELS
            ...
            with tqdm(valid_dl, leave=True, desc=f"Validation") as pbar:
                ...
                val_wll = criterion2(y_preds, labels) / model_config.N_LABELS
